<script setup lang="ts">
	...
	const listAPI = ref<ExchangeListAPI>();

	// api
	Api.getAll([
		{ key: `ORDER_EXCHANGE_LIST`, ref: listAPI },
	]);
...
<template>
	...
	<None
		v-if="listAPI != null && listAPI.exchange_orders.length < 1"
	>
		교환 내역이 없습니다
	</None>
	...
	>
		<article
			v-for="order in listAPI?.exchange_orders"
			:key="order.order_id"
			class="mm_order-item"
		>
			<h5>
