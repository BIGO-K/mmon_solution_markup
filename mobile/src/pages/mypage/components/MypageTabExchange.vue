<script setup lang="ts">
	...
	const exchangeListAPI = ref<ExchangeListAPI>();

	// Api를 클래스로 변경하고 load, change 등 별도 함수로 분리??
	Api.get<ExchangeListAPI>(`ORDER_EXCHANGE_LIST`, exchangeListAPI);
...
<template>
	<p
		v-if="(exchangeListAPI?.exchange_orders.length ?? NaN) < 1"
		class="mm_text-none"
	>
	...
	>
		<article
			v-for="order in exchangeListAPI?.exchange_orders"
			:key="order.order_id"
			class="mm_order-item"
		>
			<h5>
						...
						>
							<div class="mm_product-item">
								...
								<div class="mm_product-item-footer">
									...
									<div
										v-if="item.to_return_switchable || item.exchange_cancelable || item.return_delivery_trackable"
										class="mm_btn_box"
									>
